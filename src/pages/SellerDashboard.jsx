...
const q = query(collection(db, 'photos'), where('sellerId','==', user.uid), orderBy('createdAt','desc'))
const snap = await getDocs(q)
const arr = snap.docs.map(d => ({ id: d.id, ...d.data() }))
setMyPhotos(arr)
const sold = arr.reduce((s,p)=> s + (p.salesCount||0), 0)
const earnings = arr.reduce((s,p)=> s + ((p.salesCount||0) * (p.price||0)), 0)
...
<StatsCard label="Most Sold" value={stats.sold} />
...
<input className="border rounded-lg px-3 py-2" type="number" placeholder="Price (paise)" value={price} onChange={e=>setPrice(e.target.value)} />
<input className="border rounded-lg px-3 py-2" type="file" accept="image/*" onChange={e=>setFile(e.target.files[0])} />
<button className="sm:col-span-2 px-4 py-2 rounded-lg bg-gray-900 text-white">Upload</button>
</form>
