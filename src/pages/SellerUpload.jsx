...
// ✅ FIXED: Now saves to "items" collection (same as samples)
...
export default function SellerUpload() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // ✅ Count seller's items in "items" collection
  ...
  const onUpload = async () => {
    ...
    try {
      ...
      // ✅ FIXED: Create item document (same structure as samples)
      const itemId = `sellers-${user.uid}-${Date.now()}-${safeName}`;

      await setDoc(doc(db, "items", itemId), {
        // ✅ Match sample item structure
        fileName: safeName,
